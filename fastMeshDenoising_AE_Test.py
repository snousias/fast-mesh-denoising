...
import fastMeshDenoising_Data_Utils_Test as dat
from fastMeshDenoising_Data_Utils_Test import *
...
if True:
    ...
    rotAx=[ mModelToProcess.faces[r].rotationAxis for r in range(0, np.size(test_images_original, axis=0))]
    thetas=[-mModelToProcess.faces[r].theta for r in range(0, np.size(test_images_original, axis=0))]
    ...
    saver2.restore(sess, reportPath + "model_"+str(numOfElements)+".ckpt")
    ...
    for g in range(0, nIterTest):
        ...
        result = np.asarray(2.0 * x_pred[:, 0:3] - 1.0 * np.ones(np.shape(x_pred[:, 0:3])))
        print("Transform complete " + str(time.time() - t))
        tottime = tottime + (time.time() - t)
    tottime = tottime / nIterTest

    if doRotate:
        # result = list(map(lambda r: rotate(result[r, :], mModelToProcess.faces[r].rotationAxis,-mModelToProcess.faces[r].theta),
        #        range(0, np.size(result, axis=0))))
    ...
    sess.close()
    print("Full time : "+ dat._modelName + " "+str(dat.numOfElements)+" "+str(tottime))



    updateVerticesWithNormals(mModelToProcess, result, 20)
    print("Step 2 complete " + str(time.time() - t))
    exportObj(mModelToProcess, dat.rootdir+'Results/denoised_0.obj')
    ...
    with open(dat.rootdir+'Results/Comparisons/Denoised/AE/' + dat._modelName + '_axis' + dat.noiseLevelAsString + '_' + str(
                  dat.numOfElements) + '.csv','w') as writeFile:
        for j in range(0, np.size(result, axis=0)):
            line = str(rotAx[j,0]) + "," + str(rotAx[j,1]) + "," + str(rotAx[j,2])
            writeFile.write(line)
            writeFile.write('\n')


    with open(dat.rootdir+'Results/Comparisons/Denoised/AE/' + dat._modelName + '_thetas' + dat.noiseLevelAsString + '_' + str(
                  dat.numOfElements) + '.csv','w') as writeFile:
        for j in range(0, np.size(result, axis=0)):
            line = str(thetas[j])
            writeFile.write(line)
    ...
    with open(
            dat.rootdir+'Results/Comparisons/Denoised/AE/' + dat._modelName + '_normals' + dat.noiseLevelAsString + '_' + str(
                    dat.numOfElements) + '.csv', 'w') as writeFile:
        for j in range(0, np.size(result, axis=0)):
