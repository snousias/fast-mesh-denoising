...
def BilateralNormalFiltering(Geom, faceNormals,sigma2,neighbours, nIter):

    sigma1 = np.zeros(len(Geom.faces))
    if np.shape(faceNormals)[0] == len(Geom.faces):

        for idx_f, f in enumerate(Geom.faces):
            for fi in f.neighbouringFaceIndices:
                sigma1[idx_f] = sigma1[idx_f] + np.linalg.norm(f.centroid - Geom.faces[fi].centroid)
            sigma1[idx_f] = sigma1[idx_f] / len(f.neighbouringFaceIndices)

        for iteration in range(nIter):
            for idx_f, f in enumerate(Geom.faces):
                casumnb = np.zeros(3)
                canormalizer = 0
                currentFaceNormal=faceNormals[idx_f, :]
                p,r = neighboursByFace(Geom,idx_f,neighbours)
                p.pop(0)
                for fi in p:
                    cat = np.linalg.norm(f.centroid - Geom.faces[fi].centroid)
                    # cah = np.linalg.norm(f.faceNormal - Geom.faces[fi].faceNormal)
                    cah = np.linalg.norm(faceNormals[idx_f, :] - faceNormals[fi, :])
                    cawc = np.exp(-(cat ** 2) / (2 * (sigma1[idx_f] ** 2)))
                    caws = np.exp(-(cah ** 2) / (2 * (sigma2 ** 2)))
                    caweight = cawc * caws * f.area
                    casumnb = casumnb + caweight * faceNormals[fi, :]
                    canormalizer = canormalizer + caweight
                faceNormals[idx_f, :] = casumnb / canormalizer

                    #faceNormalsUpdated[idx_f, :] =fupdated
            #faceNormals=faceNormalsUpdated
            #print(np.linalg.norm(faceNormalsUpdated-faceNormals))
